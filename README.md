...
│   ├── exp2_calibration.py  # Calibration count × 5 placement strategies (incl. bridge)
...
│   ├── exp10_feedback_loop.py   # 4-condition adversarial retraining loop
│   ├── exp_centrality.py    # Bridge score (betweenness × cross-group fraction) vs h
│   └── exp_degree_dist.py   # Degree distribution: degree-matched SBM vs ER / BA / WS
├── visualizations/
...
├── demo.html                # Self-contained browser demo (no server required)
...
# 3. Run all experiments  (~5–8 min, 200 Monte Carlo trials, parallelised via joblib)
python run_all.py

# Fast test run (20 trials, still saves plots)
python run_all.py --fast

# Custom run count
python run_all.py --runs 50

# Run specific experiments only
python run_all.py --exp 1 6 7

# 4. Launch Streamlit interactive demo
streamlit run app.py

# 5. Open static browser demo (no server needed)
open demo.html

# 6. Export Gephi files
python visualizations/gephi_export.py
```

> **Conda users:** `conda activate course && python run_all.py`
...
|---|--------|-----------------|-------------|
| 1 | `exp1_topology.py` | K × p_out heatmap | Optimal K* minimises peak infection; K=4–6 is most vulnerable — too many or too few squads both reduce cascade risk |
| 2 | `exp2_calibration.py` | Calibration count × 5 placement strategies | Concentrated in the attacked group is best (final 0.083 at 10 nodes). Bridge-score placement (betweenness × cross-group fraction) scores 0.436 — structural centrality is not a substitute for local peer pressure |
| 3 | `exp3_attack.py` | Attack seed count vs infection cascade | Clear cascade threshold at 8–12 seeds; concentrated attacks are ~75% more efficient than scattered at triggering collapse |
| 4 | `exp4_dynamic_defense.py` | Severance threshold h_sev sweep | Optimal h_sev = 0.40 cuts peak infection 0.741 → 0.270 (64% reduction); too aggressive severs recovery pathways |
| 5 | `exp5_alpha_beta.py` | α/β weight sensitivity surface | High α (in-group bias) keeps errors local but sticky; high β accelerates both spread and recovery |
| 6 | `exp6_phase_diagram.py` | 2D phase diagram: homophily × attack intensity | Defines the exact h_crit boundary separating recovery from collapse; sharp phase transition |
| 7 | `exp7_spectral.py` | Algebraic connectivity λ₂ + Cheeger constant | **Pearson r = −0.906, p = 0.0001** — strongest result in project. Lower λ₂ = slower mixing = errors trapped in squad |
| 8 | `exp8_scalability.py` | (a) N=50→500; (b) SBM vs ER/BA/WS | Cascade dynamics are robust across swarm sizes; SBM produces qualitatively distinct behaviour from scale-free (BA) graphs |
| 9 | `exp9_disparity.py` | ΔInfection = squad-0 rate − other-squad rate | High homophily traps misinformation in the attacked squad; disparity peaks at ΔU ≈ 0.109 at h ≈ 40 |
| 10 | `exp10_feedback_loop.py` | 4-condition adversarial retraining loop | Adaptive + learning-adversary reduces system disparity 0.394 → 0.075 (81% reduction at h=4). Static defense diverges when adversary retargets |
| B | `exp_centrality.py` | Bridge score (betweenness × cross-group fraction) | At high h, bridge scores concentrate on inter-group nodes; these become the basis for the bridge calibration strategy in Exp 2 |
| D | `exp_degree_dist.py` | Degree-matched SBM vs ER / BA / WS | After correcting for degree (E[deg] ≈ 6 across all models), SBM exhibits bimodal clustering reflecting community structure absent in ER/BA/WS |

### Experiment 10 — Adversarial Retraining Loop (detail)

Exp 10 models an adversary that **retargets** based on observed infection:

```
k*(t+1) = argmin_k  ρ_k(t)      # attack the least-infected squad
```

Four conditions are tested across T=10 feedback rounds:

| Condition | Defense | Adversary | Final disparity (h=4) |
|-----------|---------|-----------|----------------------|
| Static + Fixed | fixed θ | fixed target | 0.394 |
| Adaptive + Fixed | θ tightens each round | fixed target | 0.160 |
| Static + Learning | fixed θ | retargets | 0.375 |
| **Adaptive + Learning** | θ tightens each round | retargets | **0.075** |

---

## Statistical Methods

All experiments use **200 independent Monte Carlo trials** (parallelised via `joblib`).
...
- **Pearson correlation** — used in Exp 7 to quantify λ₂ vs peak infection (r = −0.906, p = 0.0001)
...
| `exp2_calibration.png` | 5-strategy calibration comparison (incl. bridge) |
...
| `exp9_disparity.png` | Inter-group disparity |
| `exp10_feedback_loop.png` | 4-condition adversarial retraining |
| `exp_centrality.png` | Bridge score distributions |
| `exp_degree_dist.png` | Degree distribution (degree-matched) |
